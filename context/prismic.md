...
- The generated local registry contains Hero, AudienceAccordion, WhoIWorkWith, HeadingAndText, ImageAndText, InformationGrid, LocationMap, FaqAccordion, ConsultationForm, CallToAction, Content, Image, and Video.
- AudienceAccordion is available on Page documents. Its repeatable audience items contain an anchor ID, audience name, summary, heading, paragraph-only Rich Text body, labeled CTA link, and image. The renderer keeps one panel open at a time, supports direct `#parents`, `#teens`, and `#adults` links, and provides arrow-key movement between its toggles.
- HeadingAndText has Side by Side, Two Columns, and Conversation Chapter variations. Every variation can opt into the shared wine surface; Side by Side retains its optional editorial link, while Conversation Chapter adds a dedicated accent statement field. All body and statement fields render through the shared Rich Text component.
...
- LocationMap has a Heading, paragraph-only Rich Text Body, Address, and labeled Map Link. The renderer derives the embedded Google Maps query from the address rather than storing iframe markup in Prismic.
- FaqAccordion has a Heading and a repeatable FAQ Items group. Each item contains a Question and paragraph-only Rich Text Answer rendered with native disclosure semantics.
- ConsultationForm contains editor-owned section headings, guidance, consent, validation/delivery messages, success copy, and a labeled privacy link. Its semantic field labels and fixed option values remain application-owned because they define the validated submission contract. The slice renders reusable form controls through the shared Zod schema and server route boundary.
...
- Hero has three pushed variations. Homepage Hero contains a plain-text heading, Rich Text lead, Rich Text supporting copy, repeatable CTA links, Rich Text helper copy, and animation poster image. Full Image Homepage Hero preserves that copy structure around a dedicated background image. Inner Page Hero contains a plain-text heading, Rich Text lead, Rich Text supporting copy, and image. All three render their heading as the page H1 outside Rich Text and send all other formatted text through the shared `PrismicRichText` component.
...
- Home, About, Therapy & Support, and Get Started use HeadingAndText for their prototype editorial copy sections. Home, About, and Therapy & Support use InformationGrid followed by CallToAction. Get Started uses InformationGrid followed by the published LocationMap and FaqAccordion sections; its FAQ rows remain editor-repeatable.
